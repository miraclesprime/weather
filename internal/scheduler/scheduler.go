	...
	for _, city := range cfg.DefaultCities {
		...
		go func() {
			...
			if r, err := weather.FetchOpenMeteo(city); err == nil && r != nil {
				...
				log.Printf("open-meteo success for %s: temp=%.2f", city, r.Temperature)
			} else if err != nil {
				log.Printf("open-meteo error for %s: %v", city, err)
			}
			// fetch from openweathermap (may fail if no key)
			if r, err := weather.FetchOpenWeatherMap(city); err == nil && r != nil {
				results = append(results, r)
				log.Printf("openweathermap success for %s: temp=%.2f", city, r.Temperature)
			} else if err != nil {
				log.Printf("openweathermap error for %s: %v", city, err)
			}

			if len(results) == 0 {
				log.Printf("no results for city %s", city)
				return
			}
			...
			store.Save(city, agg)
			log.Printf("saved aggregated weather for %s: temp=%.2f at %s", city, agg.Temperature, agg.Time.Format(time.RFC3339))
		}()
	}
}
